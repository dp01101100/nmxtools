...
C     for Taurus v2.x systems, when GPS satellite constellation starts
C     broadcasting upcoming leap second, datalogger jerks the clock forward,
C     causing a time tear.  To fix, change the header times in all mseed
C     blockettes starting with the tear time back by one second.  Program sets
...
C        -s yy mo dy hr mn ss ms - start fixing time stamps up to this
C           time, to either 23:59:60 on June 30 or Dec. 31.
...
C         23 Aug. 2012
C     last update 9 Sep. 2013
      parameter (iblk=8192,ibmx=iblk-1)
      character buf(0:ibmx), str*(iblk)
      integer data(1000), stim(7), etim(6), dt
      character fn*128, strm*6, pm*2
      equivalence (buf,str)
      logical ohdr, odat, ochr, ok, otrs, ouse
...
C     Parse options
	 ...
	 if (fn .eq. '-s') then
	    do j=1,7
	       call getarg(i+j,fn)
	       if (fn .eq. ' ') stop '**Missing -s value'
	       read(fn,*,iostat=ios) stim(j)
	       if (ios.ne.0) stop '**Bad -s value'
	    enddo
C           Compute nearest occurrence of future leap second
	    etim(1) = stim(1)
            if (stim(2) .lt. 7) then
	       etim(2) = julday(stim(1),6,30) - julday(stim(1),1,0)
	    else
	       etim(2) = julday(stim(1),12,31) - julday(stim(1),1,0)
	    endif
	    etim(3) = 23
	    etim(4) = 59
C           Convert start time to yyyy jday hr mn ss ms
	    jday = julday(stim(1),stim(2),stim(3))-julday(stim(1),1,0)
	    stim(2) = jday
	    do j=3,6
	       stim(j) = stim(j+1)
	    enddo
...
C     End time depends on direction of leap second
      etim(5) = 59+idir
      etim(6) = 00

      if (.not.odat) stop '**No -o output file given'
      if (.not.ohdr) stop '**No -s time given'
...
C     Report station info and block size.
      call mtime(buf, irate, iyear, ijday, ihr, imn, isc, ims)
...
10    continue
	 ...
	 ix = index(fn,' ')-1
C        Calculate time of end of buffer with respect to leap second.  If
C        buffer includes or runs up to a leap second, flag it in the blockette
C        activity flags.
	 buft = bufdt(buf)
	 bufc = (buft-int(buft))*10 000
	 isec = int(buft) + int(bufc + ims)/10 000
	 itrm = dt(iyear, ijday, ihr, imn, isc+isec+ileap, etim)
	 if (itrm.ge.0 .and. ileap.ne.0) then
...
C           to flag leap second occurrence.
     ...
     &      ' start ', iyear, ijday, ihr, imn, isc, ims,' -> ',fn(1:ix)
         ...
         call mtput(buf, irate, iyear, ijday, ihr, imn, isc, ims)
...
99    continue
      ...
      subroutine usage
      write(0,*) 'Usage: tv2msleapfix -o <file>'
      write(0,*) '          -s yy mo dd hh mn ss ms '
      write(0,*) '          [-t | -l {+1 | -1} | -h]...'
      write(0,*) ' where'
      write(0,*) ' -o <file> - output file for repaired blockettes'
      write(0,*) ' -s yy mo dd hh mn ss ms - start time of time tear'
